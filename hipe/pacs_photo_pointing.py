...
for i in range(len(col)):
	obs = getObservation(instrument='PACS',obsid=col[i],useHsa=True)
	pp=obs.auxiliary.pointing
	calTree=getCalTree(obs=obs)
	orbitEphem = obs.auxiliary.orbitEphemeris
	sliced = obs.level1.refs["HPPAVGB"].product
	frames = photAddInstantPointing(sliced, pp, calTree=calTree, orbitEphem=orbitEphem, copy=1)
